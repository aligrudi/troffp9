...
void drawspline(struct ustr *ustr, int flag)
{
	int x[100], y[100];
	int i, N;

	for (N = 2; N < sizeof(x) / sizeof(x[0]); N++)
		if (ustr_int(ustr, &x[N]) <= 0 || ustr_int(ustr, &y[N]) <= 0)
			break;
...
void draw(struct ustr *ustr)
{
	...
	case 'a':
		if (ustr_int(ustr, &x1) <= 0 || ustr_int(ustr, &y1) <= 0 ||
			ustr_int(ustr, &x2) <= 0 || ustr_int(ustr, &y2) <= 0)
			error(FATAL, "draw arc function, coordinates not found.\n");
		endstring();
		if (pageon())
			fprintf(fout, "%d %d %d %d %d %d Da\n",
 ...
 *
 */
void beginpath(char *buf, int copy)
{
 ...
 *
 */
void drawpath(char *buf, int copy)
{
	if (inpath) {
		if (copy)
			fprintf(fout, "%s\n", buf);
	...
	for (; p != NULL; p = q ) {
		...
		if (strcmp(p, "stroke") == 0) {
			...
			if (q) {
				...
				if (q = strchr(p, ' '))
					*q++ = '\0';
				fprintf(fout, "%s resolution mul 2 div setlinewidth\n", p);
			}
		} else if (strncmp(p, "reverse", strlen("reverse")) == 0)
			fprintf(fout, "reversepath\n");
		else if (*p == '"') {
